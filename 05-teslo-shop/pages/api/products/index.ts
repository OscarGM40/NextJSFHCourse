...
async function getProducts(req: NextApiRequest, res: NextApiResponse<Data>) {
  ...
  await db.connect();
  if (req.query['gender'] && SHOP_CONSTANTS.validGenders.includes(req.query['gender'] as string)) {
    products = await ProductModel.find({ gender: req.query['gender'] })
      .select('title images price gender inStock slug -_id')
      .lean();
  } else if(req.query['gender'] && !SHOP_CONSTANTS.validGenders.includes(req.query['gender'] as string)) {
